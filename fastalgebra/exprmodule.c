...
static PyObject *Expression_evaluate(PyObject *self, PyObject *args){
  PyObject *d;
  if(!PyArg_Parse(args, "O!", &PyDict_Type, &d)){
    return NULL;
  }
  double r = expr_evaluate(&((Expression*)self)->expression, d);
  if (PyErr_Occurred()){
    return NULL;
  }
...
static struct PyMethodDef Expression_methods[] = {
    ...
    {"evaluate", Expression_evaluate, METH_O,
        "evaluates an expresssion"},
    {NULL, NULL, 0, NULL}};
